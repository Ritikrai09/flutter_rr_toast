...
import 'package:flutter_rr_toast/src/widget_position.dart';
import 'initiate.dart';
import 'navigator_observer.dart';
import 'rr_init.dart';
import 'rr_toast_widget.dart';
...
class RRToast {
  ...
  static CancelFunc showCustomNotification(
    ...
    return showAnimationWidget(
        ...
        animationReverseDuration: animationReverseDuration,
        wrapAnimation: wrapAnimation,
        wrapToastAnimation: (controller,dx,dy,cancel, child) {
          if (wrapToastAnimation != null) {
            child = wrapToastAnimation(controller,dx,dy,cancel, child);
          }
          if (align != null) {
        ...
        toastBuilder: (cancelFunc) => NotifyToast(
            dismissDirections: dismissDirections,
  ...
  static CancelFunc showCustomText(
    ...
    return showAnimationWidget(
        ...
        animationReverseDuration: animationReverseDuration,
        wrapAnimation: wrapAnimation,
        wrapToastAnimation: (controller, dy, dx, cancel, child) {
          if (wrapToastAnimation != null) {
            child = wrapToastAnimation(controller,dy, dx, cancel, child);
          }
          if (align != null) {
            child = Align(alignment: align, child: child);
          }
  ...
  static CancelFunc showAttachedWidget(
    ...
    return showAnimationWidget(
        ...
        duration: duration,
        wrapAnimation: wrapAnimation,
        wrapToastAnimation: (controller,dx,dy,cancel, child) => CustomSingleChildLayout(
        ...
        warpWidget: (cancel, child) => ProxyInitState(
              ...
              child: ProxyDispose(
                  ...
                  child: wrapAnimation != null
                      ? wrapAnimation(controller!,dx ?? 0,dy ?? 0.5,cancel, child)
                      : child),
            ),
        toastBuilder: (cancelFunc) => wrapToastAnimation != null
            ? wrapToastAnimation(
                controller!,dx ?? 0,dy ?? 0.5, cancelFunc, toastBuilder(cancelFunc))
  ...
  static CancelFunc showEnhancedWidget(
    ...
    //跨页自动关闭
    RRNavigatorObserverProxy? observerProxy;
    if (!crossPage) {
      observerProxy = RRNavigatorObserverProxy.all(dismissFunc);
      RRNavigatorObserver.register(observerProxy);
    }
    ...
    if (backButtonBehavior == BackButtonBehavior.ignore) {
      unRegisterFunc =
          RRToastWidgetsBindingObserver.singleton.registerPopListener(() {
        return true;
      });
        ...
        toastBuilder: (_) {
          return  ProxyDispose(disposeCallback: () {
              ...
              if (observerProxy != null) {
                RRNavigatorObserver.unregister(observerProxy);
              }
              timer?.cancel();
              onClose?.call();
              unRegisterFunc?.call();
  ...
  static CancelFunc showWidget(
    ...
    rrToastManager.insert(gk, uniqueKey, toastBuilder(cancelFunc));
    return cancelFunc;
  }

  static void remove(UniqueKey key, [String? groupKey]) {
    rrToastManager.remove(groupKey ?? defaultKey, key);
  }

  static void removeAll([String? groupKey]) {
    rrToastManager.removeAll(groupKey ?? defaultKey);
  }

  static void cleanAll() {
    rrToastManager.cleanAll();
  }
